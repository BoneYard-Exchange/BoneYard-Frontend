...
import { fetchLockedKingdomUserData, fetchPoolVaultData } from '../../views/Kingdoms/LockedKingdom/poolHelpers'
...
export const fetchFarmUserAllowances = async (account: string, farmsToFetch: FarmConfig[]) => {
  ...
  const calls = farmsToFetch.map((farm) => {
    ...
    const lpContractAddress =
      farm.isTokenOnly || farm.isKingdomToken ? getAddress(farm.token.address) : getAddress(farm.lpAddresses)
...
async function getLockedKingdomsUserEarnings(account: string) {
  ...
  return getCakeVaultEarnings(
    account,
    new BigNumber(poolVaultUserData.tokenAtLastUserAction),
    new BigNumber(poolVaultUserData.shares),
    new BigNumber(poolVaultData.pricePerFullShare),
    0,
    new BigNumber(poolVaultData.fees.performanceFee),
  ).autoCakeToDisplay
}
