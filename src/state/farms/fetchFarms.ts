    ...
    farmsToFetch.map(async (farmConfig) => {
      ...
      if (farmConfig.isTokenOnly || farmConfig.isKingdomToken) {
        tokenAmount = farmConfig.isKingdomToken
          ...
          : new BigNumber(lpTokenBalanceMC).div(new BigNumber(10).pow(tokenDecimals))
      ...
      const [info, totalAllocPoint, boydPerBlock] = await multicall(masterchefABI, mCalls)
      ...
      if (farmConfig.isKingdom && !farmConfig.isKingdomLocked) {
        ...
        const kingdomCorrectAlloc = allocPoint
          .times(new BigNumber(kingdomTotalAlloc))
          .div(new BigNumber(kTotalAllocPoint))
        ...
        return {
          ...
          boydPerBlock: new BigNumber(boydPerBlock).toNumber(),
