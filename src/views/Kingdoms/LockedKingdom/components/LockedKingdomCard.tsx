...
import { Button as UiButton, Flex, Heading, Text, useModal } from '@boneyard/uikit'
...
const LockedKingdomCard: React.FC<KingdomCardProps> = ({
  ...
  const { tokenBalance: tokenBalanceAsString = 0, allowance: allowanceAsString = 0 } = farm.userData || {}
  ...
  // stake is locked?
  const isStakeLocked =
    (userDataAsBigNumbers?.lockEndTime.gte(new Date().getTime() / 1000) &&
      userDataAsBigNumbers?.lockEndTime.toNumber() !== 0) ||
    false
  ...
  const [onPresentMoreDepositLocked] = useModal(
    <DepositModalLocked
      isAddAdditional
      currentStartTime={userDataAsBigNumbers?.lockStartTime?.toNumber() || 0}
      currentEndTime={userDataAsBigNumbers?.lockEndTime?.toNumber() || 0}
      max={tokenBalance}
      onConfirm={onStakeLocked}
      tokenName={tokenName}
      addLiquidityUrl={addLiquidityUrl}
      isTokenOnly={isTokenOnly}
      isKingdomToken={isKingdomToken}
    />,
  )
  ...
  const existingStakeDuration =
    userDataAsBigNumbers?.lockEndTime?.minus(userDataAsBigNumbers?.lockStartTime).div(604800).toNumber() || 0
  const existingStakeSecondsRemain = Math.max(
    userDataAsBigNumbers?.lockEndTime?.minus(new Date().getTime() / 1000).toNumber() || 0,
    0,
  )
  ...
  const [onPresentExtend] = useModal(
    <ExtendModal
      amount={stakedBalance.toString()}
      existingStakeDuration={existingStakeDuration}
      existingStakeSecondsRemain={existingStakeSecondsRemain}
      onConfirm={onStakeLocked}
      tokenName="BOYD"
      addLiquidityUrl={addLiquidityUrl}
      title="Extend"
  ...
  const [onPresentRenew] = useModal(
    <ExtendModal
      amount={autoCakeToDisplay.toString()}
      existingStakeDuration={existingStakeDuration}
      onConfirm={onStakeLocked}
      tokenName="BOYD"
      addLiquidityUrl={addLiquidityUrl}
  ...
  const [onPresentFlexAdd] = useModal(
    <DepositModal
      ...
      isTokenOnly={isTokenOnly}
      isKingdomToken={isKingdomToken}
      onConvertToLocked={onPresentConvertToLocked}
      showConvertToLocked
      maxLockedApy={maxLockedApy}
    />,
  )
  ...
  const [onPresentWithdraw] = useModal(
    <WithdrawModal
      shares={userDataAsBigNumbers?.shares}
      performanceFee={userDataAsBigNumbers?.performanceFee
        .plus(userDataAsBigNumbers?.overdueFee)
        .plus(userDataAsBigNumbers?.userBoostedShare)}
      hasWithdrawFee={userDataAsBigNumbers?.lastDepositedTime.lte(getEpochSecondsIn3Days())}
      pricePerFullShare={poolVaultData?.pricePerFullShare}
      onConfirm={onUnstake}
      tokenName={tokenName}
      isTokenOnly={isTokenOnly}
      isKingdomToken={isKingdomToken}
  ...
  function renderRightInnerPanelContent() {
              ...
              <ActionContent style={{ flexWrap: 'wrap' }}>
                ...
                <ActionContent style={{ flexDirection: 'column', alignItems: 'flex-end' }}>
                  ...
                  <Heading color="text" size="md" mb="8px">
                    {formatDistanceToNow(new Date((userDataAsBigNumbers?.lockEndTime?.toNumber() || 0) * 1000), {
                      addSuffix: true,
                    }).replace(/^in /, '')}{' '}
                    {/* questionmark tooltip here */}
                    <abbr
                      title={`After Burning starts at ${format(
                        new Date((userDataAsBigNumbers?.lockEndTime?.toNumber() || 0) * 1000 + 604800000),
                        'MMM dd yyyy, HH:mm',
                      )}. You need to renew your fix-term position, to initiate a new lock or convert your staking position to flexible before it starts. Otherwise all the rewards will be burned within the next 90 days.`}
                    >
                      ?
                    </abbr>
                  </Heading>
                  ...
                  <Text fontSize="12px" color="textSubtle">
                    On{' '}
                    {format(
                      new Date((userDataAsBigNumbers?.lockEndTime?.toNumber() || 0) * 1000),
                      'MMM dd yyyy, HH:mm',
                    )}
                  </Text>
          ...
          return (
            <>
              <ActionContent style={{ flexWrap: 'wrap' }}>
                ...
                {afterBurnData.isAfterBurning ? (
                  <ActionContent style={{ flexDirection: 'column', alignItems: 'flex-end' }}>
                    <ActionTitles>
                      <Subtle style={{ color: 'lightgray' }}>AFTER BURNING</Subtle>
                    </ActionTitles>
                    <Text color="failure" bold>
                      {userDataAsBigNumbers?.overdueFee?.toNumber() > 0 && cakeVaultEarnings?.autoCakeToDisplay > 0
                        ? `${userDataAsBigNumbers?.overdueFee
                            ?.div(DEFAULT_TOKEN_DECIMAL)
                            ?.toNumber()
                            .toFixed(3)} CUB Burned`
                        : '-'}
                    </Text>
                  </ActionContent>
                ) : (
      ...
      return (
        <>
          ...
          <ActionContent style={{ flexWrap: 'wrap' }}>
            ...
            <div style={{ width: '10%' }} />
            <Button mt="8px" fullWidth onClick={onPresentDepositLocked}>
              Locked
            </Button>
            <div style={{ width: '100%', flex: '100% 0 0' }}>
              <Text>
                <abbr title="Flexible staking offers flexibility for staking/unstaking whenever you want. Locked staking offers higher APY as well as other benefits.">
    ...
    return (
      <>
        ...
        <ActionContent>{approveButton}</ActionContent>
      </>
    )
  }

  const durationSeconds =
    (userDataAsBigNumbers?.lockEndTime?.toNumber() || 0) - (userDataAsBigNumbers?.lockStartTime?.toNumber() || 0)
  const boostFactor = getBoostFactor(durationSeconds)
  const numWeeks = Math.floor(durationSeconds / 604800)

  const unstakeFreeSeconds = useMemo(
    () => getSecondsUntilFlexUnstakeIsFree(userDataAsBigNumbers, poolVaultData),
    [poolVaultData, userDataAsBigNumbers],
  )
            ...
            <ActionContent style={{ flex: '50%' }}>
              <div>
                ...
                <ActionContent>
                  <div>
                    <Heading color="text" size="md">
                      {new BigNumber(boostFactor.toString()).toFixed(3) || '0'}x
                    </Heading>
                    <Text fontSize="12px" color="textSubtle">
                      Lock for {numWeeks} week{numWeeks !== 1 ? 's' : ''}
                    </Text>
                  </div>
                </ActionContent>
              </div>
    ...
    return (
      <>
        ...
        <ActionContent style={{ flexWrap: 'wrap' }}>
          ...
          <div style={{ width: '50%', flex: '50% 0 0' }}>
            {unstakeFreeSeconds === 0 && isStakeActive ? (
              <Text>No unstaking fee</Text>
            ) : (
              <>
  ...
  function renderBottomPanelContent() {
    if (wasStakeLocked && !isStakeLocked) {
      return (
        <Message
          ...
          action={
            <Flex flexGrow={1} style={{ justifyContent: 'flex-end' }}>
              <Button variant="primary" onClick={onPresentRenew} disabled={location.pathname.includes('archived')}>
                Renew
              </Button>
              <Button
                variant="secondary"
        ...
        >
          <Text>Renew your staking position to continue earning CUB rewards.</Text>
        </Message>
      )
    }
    ...
    if (isStakeActive && !isStakeLocked) {
      return (
        ...
        >
          <Text>Lock staking users are earning up to {new BigNumber(maxLockedApy).toFixed(2)}% APY.</Text>
        </Message>
      )
    }
  ...
  return (
    <>
      <Detail style={{ flex: '40%' }}>
        <ActionContainer>{renderLeftInnerPanelContent()}</ActionContainer>
      </Detail>
      <Detail style={{ flex: '40%' }}>
...
function getSecondsUntilFlexUnstakeIsFree(userDataBigNumber, poolVaultData) {
  ...
  if (userDataBigNumber?.lastDepositedTime) {
    const secondsUntilFree = userDataBigNumber?.lastDepositedTime
      .plus(poolVaultData?.fees?.withdrawalFeePeriod || 0)
      .minus(Math.floor(new Date().getTime() / 1000))
